...
export default function ServiceCenterTasksPage() {
  ...
  const { data: clientsData } = useSWR(
    currentServiceCenterId ? ["sc-clients-for-filter", currentServiceCenterId] : null,
    async () => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage your {serviceCenterTasks.length} assigned task{serviceCenterTasks.length !== 1 ? 's' : ''}
          </p>
        </div>
                      ...
                      <tr key={taskId} className="group hover:bg-muted/50 transition-colors">
                        ...
                        <td className="py-4 text-right">
                          <Button
                            ...
                            onClick={() => router.push(`/service-center/clients/${clientId}`)}
                          >
